...
const Writer = require("./source-map-writer");
...
module.exports = function(eleventyConfig, userOptions = {}) {
  let {sass: sassOptions, ...options} = userOptions;
  ...
  const extensionOptions = Object.assign({
    ...
    compile: async function(inputContent, inputPath) {
      ...
      // Must use sass.compileString() here.
      // If you used sass.compile(), sass would touch the file and 11ty would compile it twice.
      let result = sass.compileString(inputContent, stringOptions);
      debugDev(`Has compiled ${ inputPath }`);
      let sourcemapStatement = stringOptions.sourceMap ? Writer.write(this.config, inputPath, result.sourceMap) : "";
  ...
  eleventyConfig.on("eleventy.beforeWatch", function(queue) {
    console.log("eleventy.beforeWatch queue: %O", queue);
