...
const eleventySass = function(eleventyConfig, userOptions = {}) {
  ...
  const retrieveRevHash = async function(inputPath, template) {
    debugDev("%o retrieveRevHash called", inputPath);
    ...
    if (revHash === undefined) {
      ...
      if (css === undefined) {
        ...
        if (css === undefined)
          css = null;
        compileCache.set(inputPath, css);
        debugDev("%o retrieveRevHash updated the compileCache with a compiled value", inputPath);
      }
    ...
    debugDev("%o retrieveRevHash is returning %o", inputPath, revHash);
    return revHash;
  };
