...
const eleventySass = function(eleventyConfig, userOptions = {}) {
  ...
  const compileWithCache = async function(inputContent, inputPath, bypass) {
    ...
    let css = await compile.call(this, inputContent, normalizedInputPath, sassOptions, this.config, postcss);
    if (css === undefined) {
  ...
  const retrieveRevHash = async function(normalizedInputPath, template) {
    ...
    if (revHash === undefined) {
      ...
      if (css === undefined) {
        ...
        css = await compile.call(this, inputContent, normalizedInputPath, sassOptions, template.config, postcss);
        if (css === undefined)
          css = null;
        compileCache.set(normalizedInputPath, css);
      }
    ...
    if (options.compileOptions?.permalink !== undefined) {
      ...
      eleventySassRevvedPermalink = async function(permalinkString, inputPath, template) {
        ...
        if (render !== false) {
          ...
          revHash = await retrieveRevHash.call(context, normalizedInputPath, template);
        }
      ...
      eleventySassRevvedPermalink = async function(permalinkString, inputPath, template) {
        ...
        let revHash = await retrieveRevHash.call(context, normalizedInputPath, template);
